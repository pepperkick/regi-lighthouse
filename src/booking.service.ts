...
import {
  BOOKING_ACTIVE_STATUS_CONDITION,
  BookingStatus,
} from './objects/booking-status.enum';
...
export class BookingService {
  ...
  constructor(
    ...
    for (const index in regions) {
      ...
      if (regions[index].default) this.defaultRegion = index;
    ...
    for (const index in variants) {
      ...
      if (variants[index].default) this.defaultVariant = index;
    }
  ...
  async getActiveRegionBookings(region: string): Promise<Booking[]> {
    return this.Booking.find({ region, $or: BOOKING_ACTIVE_STATUS_CONDITION });
  }

  /**
   * Get a region and tier active bookings
   */
  async getActiveRegionTierBookings(
    region: string,
    tier: string,
  ): Promise<Booking[]> {
    return this.Booking.find({
   ...
   */
  static async sendRconCommandRequest(
    booking: Booking,
    command: string,
  ): Promise<string> {
  ...
  async validateBookRequest(options: BookingOptions): Promise<boolean> {
    ...
    const userBookings = await this.getActiveUserBookings(
      this.getMemberId(options.bookingFor),
    );
    ...
    if (userReservations.length !== 0)
      throw new WarningMessage(
        await this.i18n.t('BOOKING.RESERVATION_ALREADY_EXISTS'),
      );
    ...
    if (!this.canUserAccessRegion(region, options.bookingBy))
      throw new WarningMessage(
        await this.i18n.t('REGION.RESTRICTED', {
    ...
    if (options.reserveAt && !tierConfig.allowReservation) {
      throw new WarningMessage(
        await this.i18n.t('BOOKING.RESERVE.NOT_ALLOWED'),
      );
    }
  ...
  async validateUnbookRequest(
    ...
    if (booking.status === BookingStatus.STARTING) {
      if (!options?.forSomeoneElse)
        ...
        throw new WarningMessage(
          await this.i18n.t('BOOKING.ADMIN.ONGOING', {
            args: { user: user.tag },
          }),
        );
    }
  ...
  async createBookingRequest(options: BookingOptions) {
    ...
    this.logger.log(
      `Received booking request from ${bookingBy.id} (${bookingBy.username}) for ${bookingFor.id} (${bookingFor.username}) at region ${region} (${tier}) [${variant}]`,
    );
    ...
    if (options.reserveAt) {
      ...
      const userChannel = (await this.bot.channels.fetch(
    ...
    const providers = this.getProviderFromOptions(
      tierConfig.provider,
      variantConfig.providerSize,
    );
  ...
  async createBooking(
    ...
    try {
      ...
      this.logger.debug(
        `Server request object: ${JSON.stringify(serverRequest, null, 2)}`,
      );
      ...
      if (error.response?.status === 429) {
        failOnProviderOverload &&
          (await this.messageService.editMessageI18n(
            statusMessage,
            MessageType.ERROR,
            'BOOKING.PROVIDER_OVERLOADED',
          ));
        return false;
      }
  ...
  async updateServerRequest(
    ...
    if (
      this.userHasRoleFromSlug(member, config.features.settings.serverPassword)
    ) {
      data = await this.preference.getData(
        user,
    ...
    if (
      this.userHasRoleFromSlug(member, config.features.settings.serverHostname)
    ) {
      data = await this.preference.getData(
        user,
    ...
    if (
      this.userHasRoleFromSlug(member, config.features.settings.serverTvName)
    ) {
      data = await this.preference.getData(
        user,
    ...
    data = await this.preference.getDataString(
      user,
      PreferenceService.Keys.serverMap,
    );
    if (data) {
    ...
    data = await this.preference.getDataString(
      user,
      PreferenceService.Keys.serverGitRepo,
    );
    if (data) {
    ...
    data = await this.preference.getDataString(
      user,
      PreferenceService.Keys.serverGitKey,
    );
    if (data) {
  ...
  async sendBookingDetails(user: User, options?: { noStatusMessage: boolean }) {
    ...
    if (userBookings.length === 0)
      throw new WarningMessage(
        await this.i18n.t('COMMAND.USER.RESEND.NO_BOOKING'),
      );
    else if (userBookings.length === 1) {
      if (userBookings[0].status === BookingStatus.STARTING) {
        throw new WarningMessage(
          await this.i18n.t('BOOKING.NO_DETAILS_DURING_STARTING'),
        );
      }
  ...
  searchRegions(text: string) {
    ...
    for (const i of keys) {
      ...
      if (match.length > 0) filter.push({ name: region.name, value: i });
    }
  ...
  searchTiers(region: string, text: string) {
    ...
    for (const i of keys) {
      if (i.includes(text)) filter.push({ name: i, value: i });
    }
  ...
  async sendBookingStatus(
    ...
    if (userBookings.length !== 0) {
      embed.setDescription(
        `You currently have an active booking at ${
          regions[userBookings[0].region].name
        }`,
      );
    }
    ...
    if (userReservations.length !== 0) {
      ...
      embed.setDescription(
        `You currently have a scheduled reservation at ${
          regions[userReservations[0].region].name
        } in ${text || '1 min'}`,
      );
    }
    ...
    if (filteredRegions.length === 0) {
      embed.addField(
        'No region found',
        'Could not find any region with that tag. Please try something else.',
        true,
      );
    ...
    for (const i of orderedRegions) {
      ...
      for (const t of orderedTiers) {
        ...
        const tierBookings = regionBookings.filter(
          (booking) => booking.tier === t,
        );
  ...
  async handleServerStatusChange(server: Server, status: ServerStatus) {
    ...
    if (
      [ServerStatus.CLOSED, ServerStatus.IDLE, ServerStatus.FAILED].includes(
        status,
      )
    ) {
      ...
      if (status === ServerStatus.IDLE) {
        ...
        message =
          booking.messages?.start || booking.messages?.close || undefined;
      }
      ...
      if (status === ServerStatus.IDLE) {
        await this.sendServerDetailsViaDM(user, booking, server, {
          statusMessage,
        });
        await this.messageService.editMessageI18n(
          statusMessage,
          MessageType.SUCCESS,
          'BOOKING.START_SUCCESS',
        );
        ...
        if (
          server.game === Game.TF2 &&
          server.provider.includes('binarylane')
        ) {
          const rcon = new Rcon({
            host: server.ip,
            port: server.port,
          ...
          await rcon.connect();
          await rcon.send(
            'logstf_api_url "http://dev.api.qixalite.com/services/logstf"',
          );
          await rcon.disconnect();
        }
      } else if (status === ServerStatus.CLOSED) {
  ...
  async cancelReservation(user: User) {
    ...
    if (reservations.length === 0) {
      throw new WarningMessage(
        await this.i18n.t('COMMAND.USER.UNRESERVE.USER_HAS_NO_RESERVATION'),
      );
   ...
   */
  async monitor() {
    const bookingReservations = await this.Booking.find({
      status: BookingStatus.RESERVED,
    });
    ...
    for (const booking of bookingReservations) {
      ...
      const reserve = moment(booking.reservedAt)
        .subtract(earlyStart, 'seconds')
        .toDate();
  ...
  async getRegions(region: string): Promise<Regions> {
    ...
    for (const i of keys) {
      ...
      const regionBookings = activeBookings.filter(
        (booking) => booking.region === i,
      );
      const tiers = region.tiers;
  ...
  async isRegionTierAvailable(region: string, tier: string): Promise<boolean> {
    const tierConfig = this.getTierConfig(region, tier);
    const runningBookings = await this.getActiveRegionTierBookings(
      region,
      tier,
    );
  ...
  userHasRoleFromSlug(
    ...
    if (access.includes('|')) {
      ...
      for (const part of parts) {
        if (this.userHasRoleFromSlug(member, part)) return true;
      }
    }
    ...
    while (i--) {
      switch (access.charAt(i)) {
        case '1':
          if (this.isUserTier1(member)) return true;
          break;
        case '2':
          if (this.isUserTier2(member)) return true;
          break;
        case '3':
          if (this.isUserTier3(member)) return true;
          break;
      }
    }
  }
  ...
  isUserTier1(member: GuildMember | APIInteractionGuildMember) {
    return config.roles.premium_tier_1 === '<bypass>'
      ? true
      : this.userHasRole(member, config.roles.premium_tier_1);
  }
  ...
  isUserTier2(member: GuildMember | APIInteractionGuildMember) {
    return config.roles.premium_tier_2 === '<bypass>'
      ? true
      : this.userHasRole(member, config.roles.premium_tier_2);
  }
  ...
  isUserTier3(member: GuildMember | APIInteractionGuildMember) {
    return config.roles.premium_tier_3 === '<bypass>'
      ? true
      : this.userHasRole(member, config.roles.premium_tier_3);
  }
  ...
  async sendServerDetailsViaDM(
    ...
    try {
      await (await user.createDM()).send({
        embeds: [BookingService.buildConnectMessage(booking, server)],
      });
      ...
      if (error.message === 'Cannot send messages to this user') {
        if (options.statusMessage)
          await this.messageService.editMessageI18n(
            ...
            MessageType.ERROR,
            'BOOKING.FAILED_TO_SEND_PRIVATE_DM',
          );
        else {
          const channel = (await this.bot.channels.fetch(
            config.channels.users,
  ...
  private static buildConnectMessage(booking: Booking, server: Server) {
    ...
    const hatchPort = server.port === 27015 ? 27017 : server.port + 2;
    const hiveUrl = `https://hive.qixalite.com/?host=${encodeURI(
      server.ip,
    ...
    if (server.data.sdrEnable) {
      message.addField(
        'Original IP',
        `Do not share this unless you have connection issues\n\`${server.ip}:${server.port}\``,
      );
    }
